...
template <typename T> constexpr T OP_ABS(T x) { return x < 0 ? -x : x; }
template <typename T> constexpr bool IN_RANGE(T lhs, T rhs, T df) {
  return OP_ABS(lhs.b - rhs.b) <= df.b && OP_ABS(lhs.g - rhs.g) <= df.g &&
...
struct color_t {
  ...
  uchar toGray() const { return (r * 299 + g * 587 + b * 114 + 500) / 1000; }
};
#pragma pack()
struct color_df_t {
  color_t color;
