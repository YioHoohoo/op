...
struct Image {
  ...
  Image(int w, int h) : pdata(nullptr) { create(w, h); }
  // copy ctr
  ...
  bool read(LPCTSTR file) {
    ...
    if (hr == S_OK) {
      ...
      translate((unsigned char *)img.GetBits(), img.GetBPP() / 8,
                img.GetPitch());
      return true;
    } else {
      return false;
  ...
  iterator end() { return (iterator)pdata + width * height; }

  iterator begin() const { return (iterator)pdata; }
  iterator end() const { return (iterator)pdata + width * height; }

  void fill(unsigned int val) { std::fill(begin(), end(), val); }
...
struct ImageBin {
  ...
  unsigned char at(int y, int x) const { return pixels[y * width + x]; }
  unsigned char &at(int y, int x) { return pixels[y * width + x]; }

  unsigned char *ptr(int y) { return pixels.data() + y * width; }

  unsigned char const *ptr(int y) const { return pixels.data() + y * width; }
