...
class sharedmem {
public:
  explicit sharedmem(const wstring &name_, size_t size_)
      : _hmap(nullptr), _paddress(nullptr), _ismaped(0) {
  ...
  bool open_create(const wstring &name_, size_t size_) {
    ...
    if (!temph) {
      temph = CreateFileMappingW(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0,
                                 size_, name_.data());
      if (!temph)
        return false;
    }
  ...
  template <typename T> T *data() { return (T *)_paddress; }
