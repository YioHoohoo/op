...
struct word_t {
  ...
  void set_chars(const std::wstring &s) {
    memcpy(info._char, s.c_str(),
           min(sizeof(info._char), (s.length() + 1) * sizeof(wchar_t)));
  }
...
struct word1_t {
  ...
  bool operator==(const word1_t &rhs) {
    if (info.w != rhs.info.w || info.h != rhs.info.h ||
        info.bit_cnt != rhs.info.bit_cnt)
      return false;
...
struct Dict {
  ...
  void sort_dict() {
    ...
    std::stable_sort(
        words.begin(), words.end(), [](const word1_t &lhs, const word1_t &rhs) {
  ...
  int size() const { return info._word_count; }
