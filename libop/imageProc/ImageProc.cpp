...
long ImageProc::FindMultiColorEx(const wstring &first_color, const wstring &offset_color, double sim, long dir,
                                 wstring &retstr) {
  std::vector<color_df_t> vfirst_color;
  str2colordfs(first_color, vfirst_color);
  std::vector<wstring> vseconds;
...
// 图形定位
long ImageProc::FindPic(const std::wstring &files, const wstring &delta_colors, double sim, long dir, long &x,
                        long &y) {
  vector<Image *> vpic;
...
long ImageProc::FindColorBlock(const wstring &color, double sim, long count, long height, long width, long &x,
                               long &y) {
...
long ImageProc::FindColorBlockEx(const wstring &color, double sim, long count, long height, long width,
                                 wstring &retstr) {
  vector<color_df_t> colors;
  if (str2colordfs(color, colors) == 0) {
    bgr2binary(colors);
...
long ImageProc::OcrAuto(double sim, std::wstring &retstr) { return OCR(L"", sim, retstr); }
