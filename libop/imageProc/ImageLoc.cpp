  ...
  for (int pic_id = 0; pic_id < pics.size(); ++pic_id) {
    ...
    for (size_t i = 0; i < m_threadPool.getThreadNum(); ++i) {
      results.push_back(m_threadPool.enqueue(
          [this, dfcolor, points, pgimg, tnorm](rect_t &block, Image *pic, int use_ts_match, double sim, bool *stop) {
            // 计算最大误差
            int max_err_ct = (pic->height * pic->width - use_ts_match) * (1.0 - sim);
            for (int i = block.y1; i < block.y2; ++i) {
              for (int j = block.x1; j < block.x2; ++j) {
...
long ImageBase::FindPicExTh(std::vector<Image *> &pics, color_t dfcolor, double sim, long dir, vpoint_desc_t &vpd) {
  ...
  for (int pic_id = 0; pic_id < pics.size(); ++pic_id) {
    ...
    for (size_t i = 0; i < m_threadPool.getThreadNum(); ++i) {
      results.push_back(m_threadPool.enqueue(
          [this, dfcolor, points, pgimg, tnorm](rect_t &block, Image *pic, int use_ts_match, double sim) -> vpoint_t {
            vpoint_t vp;
            // 计算最大误差
            int max_err_ct = (pic->height * pic->width - use_ts_match) * (1.0 - sim);
            for (int i = block.y1; i < block.y2; ++i) {
...
long ImageBase::FindColorBlockEx(double sim, long count, long height, long width, std::wstring &retstr) {
  record_sum(_binary);
  int cnt = 0;
  for (int i = 0; i <= _binary.height - height; ++i) {
    for (int j = 0; j <= _binary.width - width; ++j) {
...
long ImageBase::FindStrEx(std::map<point_t, ocr_rec_t> &ps, const vector<wstring> &vstr, double sim,
                          std::wstring &retstr) {
...
*/
void ImageBase::_bin_ocr(const Dict &dict, std::map<point_t, ocr_rec_t> &ps) {
  int px, py;
  if (_binary.empty())
    return;
  //
...
// 模糊匹配 待识别区域可以含有噪声
void ImageBase::_bin_ocr(const Dict &dict, double sim, std::map<point_t, ocr_rec_t> &ps) {
  int px, py, y;
  if (_binary.empty())
    return;
  //
  ...
  for (py = 0; py < _binary.height - h_min + 1; ++py) {
    ...
    for (px = 0; px < _binary.width - w_min + 1; ++px) {
      ...
      for (auto &it : dict.words) {
        ...
        // quick check
        // error tolerance
        int error_tolerance = (1 - sim) * it.info.w * it.info.h;
        ...
        if (match_error <= error_tolerance) {
          ...
          if (rs <= it.info.h / 2) {
            ...
            ocr_res.right_bottom = point_t(crc.x2, crc.y2);
            ocr_res.text = it.info.name;
            ocr_res.confidence = (crc.area() - match_error) / (double)crc.area();
            ps[pt] = ocr_res;
            fill_rect(_record, crc);
            // break;//words
            break;
...
void ImageBase::bin_ocr(const Dict &dict, double sim, std::map<point_t, ocr_rec_t> &ps) {
  ps.clear();
  if (dict.words.empty())
    return;
  if (_binary.empty())
