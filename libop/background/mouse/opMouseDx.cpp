...
opMouseDx::~opMouseDx() { _hwnd = NULL; }
...
long opMouseDx::Bind(HWND h, int mode) {
  ...
  if (NT_SUCCESS(hr)) {
    ...
    BOOL is64 =
        proc.modules().GetMainModule()->type == blackbone::eModType::mt_mod64;
    if (is64 != OP64) {
    ...
    if (injected) {
      ...
      auto PSetInputHook = blackbone::MakeRemoteFunction<my_func_t>(
          proc, dllname, "SetInputHook");
      if (PSetInputHook) {
...
long opMouseDx::UnBind() {
  ...
  if (NT_SUCCESS(hr)) {
    ...
    auto pReleaseInputHook = blackbone::MakeRemoteFunction<my_func_t>(
        proc, dllname, "ReleaseInputHook");
...
long opMouseDx::MoveR(int rx, int ry) { return MoveTo(_x + rx, _y + ry); }
...
long opMouseDx::LeftClick() {
  ...
  ret = ::SendMessageTimeout(_hwnd, OP_WM_LBUTTONDOWN, MK_LBUTTON,
                             MAKELPARAM(_x, _y), SMTO_BLOCK, 2000, nullptr);
  // ret = ::SendNotifyMessage(_hwnd, WM_LBUTTONDOWN, MK_LBUTTON, MAKELPARAM(_x,
  // _y));
  ::Delay(MOUSE_DX_DELAY);
  // ret = ::SendMessage(_hwnd, WM_LBUTTONDOWN, MK_LBUTTON, MAKELPARAM(_x, _y));
  ret2 = ::SendMessageTimeout(_hwnd, OP_WM_LBUTTONUP, 0, MAKELPARAM(_x, _y),
...
long opMouseDx::RightDown() {
  ...
  ret =
      ::PostMessage(_hwnd, WM_RBUTTONDOWN, MK_RBUTTON, MAKELPARAM(_x, _y)) == 0
          ? 1
          : 0;
...
long opMouseDx::WheelDown() {
  ...
  ret = ::SendMessage(_hwnd, OP_WM_MOUSEWHEEL, MAKEWPARAM(-WHEEL_DELTA, 0),
                      MAKELPARAM(_x, _y)) == 0
            ? 1
            : 0;
