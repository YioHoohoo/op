...
opMouseWin::~opMouseWin() { _hwnd = NULL; }
...
long opMouseWin::MoveTo(int x, int y) {
  ...
  case INPUT_TYPE::IN_NORMAL: {
    ...
    input was already blocked by another thread. To get extended error
    information, call GetLastError. This function fails when it is blocked by
    UIPI. Note that neither GetLastError nor the return value will indicate the
    failure was caused by UIPI blocking.
    */
    ret = ::SendInput(1, &Input, sizeof(INPUT)) > 0 ? 1 : 0;
    break;
  }
  case INPUT_TYPE::IN_WINDOWS: {
    ret = ::SendMessageTimeout(_hwnd, WM_MOUSEMOVE, 0, MAKELPARAM(x, y),
...
long opMouseWin::LeftClick() {
  ...
  case INPUT_TYPE::IN_WINDOWS: {
    ret = ::SendMessageTimeout(_hwnd, WM_LBUTTONDOWN, 0, MAKELPARAM(_x, _y),
                               SMTO_BLOCK, 2000, nullptr);
    ::Delay(MOUSE_WINDOWS_DELAY);
    ret = ::SendMessageTimeout(_hwnd, WM_LBUTTONUP, 0, MAKELPARAM(_x, _y),
                               SMTO_BLOCK, 2000, nullptr);
    /// ret=::PostMessage(_hwnd, WM_LBUTTONDOWN, MK_LBUTTON, MAKELPARAM(_x,
    /// _y));
    // ret = ::SendMessageTimeout(_hwnd, WM_LBUTTONDOWN, MK_LBUTTON,
    // MAKELPARAM(_x, _y), SMTO_BLOCK, 2000, nullptr); ret =
    // ::SendNotifyMessage(_hwnd, WM_LBUTTONDOWN, MK_LBUTTON, MAKELPARAM(_x,
    // _y));
    //::Sleep(100);
    // ret = ::SendMessage(_hwnd, WM_LBUTTONDOWN, MK_LBUTTON, MAKELPARAM(_x,
    // _y)); ret2=::SendMessageTimeout(_hwnd, WM_LBUTTONUP, 0, MAKELPARAM(_x,
    // _y), SMTO_BLOCK, 2000, nullptr); ret2 = ::SendMessage(_hwnd,
    // WM_LBUTTONUP, 0, MAKELPARAM(_x, _y));
...
long opMouseWin::LeftDown() {
  ...
  case INPUT_TYPE::IN_WINDOWS: {
    ret = ::SendMessageTimeout(_hwnd, WM_LBUTTONDOWN, MK_LBUTTON,
                               MAKELPARAM(_x, _y), SMTO_BLOCK, 2000, nullptr);
    break;
  }
  }
  return ret;
...
long opMouseWin::LeftUp() {
  ...
  case INPUT_TYPE::IN_WINDOWS: {
    ret = ::SendMessageTimeout(_hwnd, WM_LBUTTONUP, MK_LBUTTON,
                               MAKELPARAM(_x, _y), SMTO_BLOCK, 2000, nullptr);
    break;
  }
  }
...
long opMouseWin::MiddleDown() {
  ...
  case INPUT_TYPE::IN_WINDOWS: {
    ret = ::SendMessageTimeout(_hwnd, WM_MBUTTONDOWN, MK_MBUTTON,
                               MAKELPARAM(_x, _y), SMTO_BLOCK, 2000, nullptr);
    break;
  }
  }
...
long opMouseWin::MiddleUp() {
  ...
  case INPUT_TYPE::IN_WINDOWS: {
    ret = ::SendMessageTimeout(_hwnd, WM_MBUTTONUP, MK_MBUTTON,
                               MAKELPARAM(_x, _y), SMTO_BLOCK, 2000, nullptr);
    break;
  }
  }
...
long opMouseWin::RightClick() {
  ...
  case INPUT_TYPE::IN_WINDOWS: {
    r1 = ::SendMessageTimeout(_hwnd, WM_RBUTTONDOWN, MK_RBUTTON,
                              MAKELPARAM(_x, _y), SMTO_BLOCK, 2000, nullptr);
    ::Delay(MOUSE_WINDOWS_DELAY);
    r2 = ::SendMessageTimeout(_hwnd, WM_RBUTTONUP, MK_RBUTTON,
                              MAKELPARAM(_x, _y), SMTO_BLOCK, 2000, nullptr);
    ret = r1 == 0 && r2 == 0 ? 1 : 0;
    break;
  }
...
long opMouseWin::RightDown() {
  ...
  case INPUT_TYPE::IN_WINDOWS: {
    ret = ::SendMessageTimeout(_hwnd, WM_RBUTTONDOWN, MK_RBUTTON,
                               MAKELPARAM(_x, _y), SMTO_BLOCK, 2000, nullptr);
    break;
  }
  }
...
long opMouseWin::RightUp() {
  ...
  case INPUT_TYPE::IN_WINDOWS: {
    ret = ::SendMessageTimeout(_hwnd, WM_RBUTTONUP, MK_RBUTTON,
                               MAKELPARAM(_x, _y), SMTO_BLOCK, 2000, nullptr);
    break;
  }
  }
...
long opMouseWin::WheelDown() {
  ...
  case INPUT_TYPE::IN_NORMAL: {
    ...
    // down
    /*
    If dwFlags contains MOUSEEVENTF_WHEEL, then dwData specifies the amount of
  ...
  case INPUT_TYPE::IN_WINDOWS: {
    ...
    A positive value indicates that the wheel was rotated forward, away from the
    user; a negative value indicates that the wheel was rotated backward, toward
    the user. The low-order word indicates whether various virtual keys are
    down. This parameter can be one or more of the following values. lParam The
    low-order word specifies the x-coordinate of the pointer, relative to the
    upper-left corner of the screen. The high-order word specifies the
    y-coordinate of the pointer, relative to the upper-left corner of the
    screen.
    */
    ...
    ::ClientToScreen(_hwnd, &pt);
    ret =
        ::SendMessageTimeout(_hwnd, WM_MOUSEWHEEL, MAKEWPARAM(0, -WHEEL_DELTA),
                             MAKELPARAM(pt.x, pt.y), SMTO_BLOCK, 2000, nullptr);
    break;
  }
  }
