...
typedef HRESULT(WINAPI *D3D12PresentPointer)(IDXGISwapChain *swapChain,
...
//         ==
//             *static_cast<const std::uintptr_t*>(static_cast<const
//             void*>(start + i))) { commandQueueOffset_ = i; break;
//         }
//         ++i;
//     }
//
...
//     SetPrivateDataInterface, GetPrivateData, GetParent).
//     // - IDXGIObject is based on IUnknown
//     //   which has 3 original virtual methods (QueryInterface, AddRef,
//     Release).
//     // So 0 + 1 + 4 + 3 = 8.
...
HRESULT opDx12Hook::CaptureFrames(HWND windowHandleToCapture,
                                  std::wstring_view folderToSaveFrames,
                                  int maxFrames) {
  ...
  if (folderToSaveFrames_.size() && *folderToSaveFrames_.rbegin() != '\\' &&
...
void opDx12Hook::CaptureFrame(IDXGISwapChain *swapChain) {
  ...
  if (readbackResource_.Get()) {
    ...
    // In a real application, probably, you will not need to save frames to a
    // file but just to place them to a buffer to generate a preview picture or
    // analyze it.
    // std::wstring filename = std::format(L"{}{}.bmp", folderToSaveFrames_,
    // frameIndex_++); MiscHelpers::SaveDataToFile(filename, bmp.data(),
    // bmp.size());
    //--
    sharedmem mem;
    promutex mutex;
    ...
    if (mem.open(DisplayHook::shared_res_name) &&
      ...
      // formatFrameInfo(pshare, DisplayHook::render_hwnd, textDesc.Width,
      // textDesc.Height); CopyImageData((char*)pshare + sizeof(FrameInfo),
      // (char*)mapText.pData, textDesc.Height, textDesc.Width, fmt);
      static_assert(sizeof(FrameInfo) == 28);

      CopyImageData((char *)pshare + sizeof(FrameInfo), (char *)readbackData_,
                    frameWidth, frameHeight, frameRowPitch, fmt);
      mutex.unlock();
    } else {
      // is_capture = 0;
#if DEBUG_HOOK
    ...
    hr = device->CreateCommittedResource(
        &readbackHeapDesc, D3D12_HEAP_FLAG_NONE, &readbackResourceDesc,
  ...
  // Create a command list to copy data from the swap chain texture to the read
  // back texture.
  ...
  D3D12_TEXTURE_COPY_LOCATION dst =
      CD3DX12_TEXTURE_COPY_LOCATION(readbackResource_.Get(), footprint);
  ...
  const char *start =
      static_cast<char *>(static_cast<void *>(swapChain3.Get()));
  ...
  // ... then execute the command list to copy the swap chain texture to the
  // read back texture.
