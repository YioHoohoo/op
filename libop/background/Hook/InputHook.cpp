...
const GUID OP_IID_IDirectInput8W = {
    0xBF798031, 0x483A, 0x4DA2, 0xAA, 0x99, 0x5D, 0x64, 0xED, 0x36, 0x97, 0x00};

const GUID OP_GUID_SysMouse = {0x6F1D2B60, 0xD5A0, 0x11CF, 0xBF, 0xC7, 0x44,
...
HRESULT __stdcall hkGetDeviceState(IDirectInputDevice8W *this_, DWORD size,
                                   LPVOID ptr);

LRESULT CALLBACK opWndProc(HWND hWnd, UINT message, WPARAM wParam,
                           LPARAM lParam);
...
int InputHook::setup(HWND hwnd_, int input_type_) {
  ...
  if (getDinputVtb() == 1) {
    MH_Initialize();
    MH_CreateHook(gDinputVtb[indexGetDeviceState], hkGetDeviceState,
                  &gDinputVtbRaw[indexGetDeviceState]);
    MH_CreateHook(gDinputVtb[indexPoll], hkPoll, &gDinputVtbRaw[indexPoll]);
    MH_EnableHook(NULL);
    gRawWindowProc = reinterpret_cast<WNDPROC>(SetWindowLongPtrA(
        hwnd_, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(opWndProc)));
...
int InputHook::release() {
  ...
  if (gRawWindowProc) {
    MH_RemoveHook(NULL);
    MH_Uninitialize();
    ptr = SetWindowLongPtrA(InputHook::input_hwnd, GWLP_WNDPROC,
                            reinterpret_cast<LONG_PTR>(gRawWindowProc));
  }
  return ptr ? 1 : 0;
  return 0;
}
...
int getDinputVtb() {
  ...
  auto pDirectInput8Create = reinterpret_cast<DirectInput8Create_t *>(
      query_api("dinput8.dll", "DirectInput8Create"));
  if (pDirectInput8Create) {
    ...
    HWND window = ::CreateWindowW(
        windowClass.lpszClassName, L"Kiero DirectX Window", WS_OVERLAPPEDWINDOW,
    ...
    if (pDirectInput8Create(GetModuleHandle(NULL), DIRECTINPUT_VERSION,
                            OP_IID_IDirectInput8W, (VOID **)&pDinput,
                            NULL) < 0) {
...
HRESULT __stdcall hkAcquire(IDirectInputDevice8W *this_) { return DI_OK; }

HRESULT __stdcall hkPoll(IDirectInputDevice8W *this_) { return DI_OK; }
...
HRESULT __stdcall hkGetDeviceState(IDirectInputDevice8W *this_, DWORD size,
  ...
  if (size == sizeof(opMouseState)) {
    ...
    return reinterpret_cast<GetDeviceState_t *>(
        gDinputVtbRaw[indexGetDeviceState])(this_, size, ptr);
  }
}
...
LRESULT CALLBACK opWndProc(HWND hWnd, UINT message, WPARAM wParam,
  ...
  return CallWindowProcA(gRawWindowProc, InputHook::input_hwnd, message, wParam,
                         lParam);
}
