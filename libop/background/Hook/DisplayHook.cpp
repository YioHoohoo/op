...
// dx10
HRESULT __stdcall dx10_hkPresent(IDXGISwapChain *thiz, UINT SyncInterval,
                                 UINT Flags);
// dx11
HRESULT __stdcall dx11_hkPresent(IDXGISwapChain *thiz, UINT SyncInterval,
                                 UINT Flags);
// dx12
HRESULT __stdcall dx12_hkPresent(IDXGISwapChain *thiz, UINT SyncInterval,
                                 UINT Flags);
// opengl_std
...
void CopyImageData(char *dst_, const char *src_, int rows_, int cols_,
                   int rowPitch, int fmt_) {
...
HRESULT dx9_capture(LPDIRECT3DDEVICE9 pDevice) {
  ...
  hr = pDevice->CreateTexture(surface_Desc.Width, surface_Desc.Height, 1, 0,
                              surface_Desc.Format,
  ...
  sharedmem mem;
  promutex mutex;
  if (mem.open(DisplayHook::shared_res_name) &&
      mutex.open(DisplayHook::mutex_name)) {
    mutex.lock();
    uchar *pshare = mem.data<byte>();
    reinterpret_cast<FrameInfo *>(pshare)->format(
        DisplayHook::render_hwnd, surface_Desc.Width, surface_Desc.Height);
    memcpy(pshare + sizeof(FrameInfo), (byte *)lockedRect.pBits,
           lockedRect.Pitch * surface_Desc.Height);
    mutex.unlock();
  }
  pTex->UnlockRect(0);
...
void dx10_capture(IDXGISwapChain *pswapchain) {
  ...
  //  If texture is multisampled, then we can use ResolveSubresource to copy it
  //  into a non-multisampled texture
  ...
  // hr = pD3DX10SaveTextureToMemory(textureDest,
  // D3DX10_IMAGE_FILE_FORMAT::D3DX10_IFF_BMP, &pblob, 0);
  if (hr < 0) {
  ...
  if (mem.open(DisplayHook::shared_res_name) &&
    ...
    mutex.lock();
    // memcpy(mem.data<char>(), mapText.pData, textDesc.Width*textDesc.Height *
    // 4);
    uchar *pshare = mem.data<byte>();
    reinterpret_cast<FrameInfo *>(pshare)->format(
        DisplayHook::render_hwnd, textDesc.Width, textDesc.Height);
    CopyImageData((char *)pshare + sizeof(FrameInfo), (char *)mapText.pData,
...
// dx10 hook Present
HRESULT STDMETHODCALLTYPE dx10_hkPresent(IDXGISwapChain *thiz,
                                         UINT SyncInterval, UINT Flags) {
  typedef long(__stdcall * Present_t)(IDXGISwapChain * pswapchain, UINT x1,
                                      UINT x2);
  if (is_capture)
    dx10_capture(thiz);
...
void dx11_capture(IDXGISwapChain *swapchain) {
  ...
  hr =
      swapchain->GetBuffer(0, __uuidof(IDXGIResource), (void **)&backbufferptr);
  if (hr < 0) {
    setlog("pswapchain->GetBuffer,error code=%X", hr);
    is_capture = 0;
    return;
  }
  hr = backbufferptr->QueryInterface(__uuidof(ID3D11Resource),
                                     (void **)&backbuffer);
  if (hr < 0) {
  ...
  // hr = pD3DX11SaveTextureToMemory(context, textureDst,
  // D3DX11_IMAGE_FILE_FORMAT::D3DX11_IFF_BMP, &pblob, 0);
  ...
  if (mem.open(DisplayHook::shared_res_name) &&
    ...
    reinterpret_cast<FrameInfo *>(pshare)->format(
        DisplayHook::render_hwnd, textDesc.Width, textDesc.Height);
    // CopyImageData((char*)pshare + sizeof(FrameInfo), (char*)mapText.pData,
    // textDesc.Height, textDesc.Width, fmt);
    static_assert(sizeof(FrameInfo) == 28);

    CopyImageData((char *)pshare + sizeof(FrameInfo), (char *)mapSubres.pData,
                  textDesc.Height, textDesc.Width, mapSubres.RowPitch, fmt);
    mutex.unlock();
  } else {
    is_capture = 0;
#if DEBUG_HOOK
    setlog("!mem.open(xhook::%s)&&mutex.open(xhook::%s)",
           xhook::shared_res_name, xhook::mutex_name);
#endif // DEBUG_HOOK
  }
  ...
  if (first) {
    ...
    setlog(
        "textDesc.Format= %d,fmt=%d textDesc.Height=%d\n textDesc.Width=%d\n  "
        "mapSubres.DepthPitch=%d\n mapSubres.RowPitch=%d\n",
        tf, fmt, textDesc.Height, textDesc.Width, mapSubres.DepthPitch,
        mapSubres.RowPitch);
    first = false;
  }
...
long gl_capture() {
  ...
  auto pglPixelStorei =
      (glPixelStorei_t)query_api("opengl32.dll", "glPixelStorei");
  auto pglReadBuffer =
      (glReadBuffer_t)query_api("opengl32.dll", "glReadBuffer");
  auto pglGetIntegerv =
      (glGetIntegerv_t)query_api("opengl32.dll", "glGetIntegerv");
  auto pglReadPixels =
      (glReadPixels_t)query_api("opengl32.dll", "glReadPixels");
  if (!pglPixelStorei || !pglReadBuffer || !pglGetIntegerv || !pglReadPixels) {
    is_capture = 0;
#if DEBUG_HOOK
    setlog("error.!pglPixelStorei || !pglReadBuffer || !pglGetIntegerv || "
           "!pglReadPixels");
#endif // DEBUG_HOOK
  ...
  if (mem.open(DisplayHook::shared_res_name) &&
    ...
    mutex.lock();
    uchar *pshare = mem.data<byte>();
    reinterpret_cast<FrameInfo *>(pshare)->format(DisplayHook::render_hwnd,
                                                  width, height);
    pglReadPixels(0, 0, width, height, GL_BGRA_EXT, GL_UNSIGNED_BYTE,
                  pshare + sizeof(FrameInfo));
    mutex.unlock();
  } else {
    is_capture = 0;
#if DEBUG_HOOK
    setlog(L"egl !mem.open(xhook::%s)&&mutex.open(xhook::%s)",
           xhook::shared_res_name, xhook::mutex_name);
#endif // DEBUG_HOOK
  }
  // setlog("gl screen ok");
...
long egl_capture() {
  ...
  auto pglPixelStorei =
      (glPixelStorei_t)query_api("libglesv2.dll", "glPixelStorei");
  auto pglReadBuffer =
      (glReadBuffer_t)query_api("libglesv2.dll", "glReadBuffer");
  auto pglGetIntegerv =
      (glGetIntegerv_t)query_api("libglesv2.dll", "glGetIntegerv");
  auto pglReadPixels =
      (glReadPixels_t)query_api("libglesv2.dll", "glReadPixels");
  if (!pglPixelStorei || !pglReadBuffer || !pglGetIntegerv || !pglReadPixels) {
    // is_capture = 0;
#if DEBUG_HOOK
    setlog(L"egl !mem.open(xhook::%s)&&mutex.open(xhook::%s)",
