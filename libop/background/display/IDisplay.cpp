...
IDisplay::IDisplay()
    : _hwnd(NULL), _shmem(nullptr), _pmutex(nullptr), _bind_state(0), _width(0),
      _height(0), _client_x(0), _client_y(0) {}
...
long IDisplay::bind_init() {
  ...
  res_size =
      (rc.right - rc.left) * (rc.bottom - rc.top) * 4 + sizeof(FrameInfo);
  ...
  try {
    ...
    setlog("bkdisplay::bind_init() %s exception:%s", _shared_res_name,
           e.what());
  }
