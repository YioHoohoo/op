...
opDXGI::opDXGI()
    : device_(nullptr), deviceContext_(nullptr), duplication_(nullptr),
      m_first(true), m_frameInfo(), dx_(0), dy_(0), m_desc() {}

opDXGI::~opDXGI() { UnBindEx(); }
...
bool opDXGI::requestCapture(int x1, int y1, int w, int h, Image &img) {
  ...
  if (texture2D == nullptr) {
    ...
    for (size_t i = 0; i < m_desc.Height; i++) {
      memcpy(pDest + i * m_desc.Width * 4, pData + i * mappedResource.RowPitch,
             m_desc.Width * 4);
    }
    _pmutex->unlock();
    texture2D->Release();
  ...
  if (src_x + w > m_desc.Width || src_y + h > m_desc.Height) {
    setlog("error w and h "
           ...
           src_x, w, m_desc.Width, src_y, h, m_desc.Height);
    return false;
  }
  ...
  // 将数据拷贝到目标
  for (int i = 0; i < h; i++) {
    // memcpy(img.ptr<uchar>(i), pDest + (desc.Height - 1 - i - src_y) * 4 *
    // desc.Width + src_x * 4, 4 * w);
    memcpy(img.ptr<uchar>(i),
           pDest + (src_y + i) * 4 * m_desc.Width + src_x * 4, 4 * w);
  }
  _pmutex->unlock();
  return true;
}
