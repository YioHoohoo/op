  ...
  if (render_type == RDT_GL_NOX) {
    ...
    if (NT_SUCCESS(hr)) {
      ...
      if (injected) {
        ...
        auto pSetXHook = blackbone::MakeRemoteFunction<my_func_t>(
            proc, dllname, "SetDisplayHook");
        if (pSetXHook) {
...
long opDxGL::UnBindEx() {
  ...
  if (NT_SUCCESS(hr)) {
    ...
    auto pUnXHook = blackbone::MakeRemoteFunction<my_func_t>(
        proc, dllname, "ReleaseDisplayHook");
    if (pUnXHook) {
      // setlog(L"bkdo::pUnXHook");
      pUnXHook();
      // BOOL fret =
      // ::FreeLibrary((HMODULE)proc.modules().GetModule(dllname)->baseAddress);
    ...
    setlog("blackbone::MakeRemoteFunction false,errcode:%X,pid=%d,hwnd=%d", hr,
           id, _hwnd);
  }
...
long opDxGL::UnBindNox() {
  ...
  if (NT_SUCCESS(hr)) {
    ...
    if (pUnXHook) {
      ...
      /*BOOL fret =
      ::FreeLibrary((HMODULE)proc.modules().GetModule(dllname)->baseAddress); if
      (!fret)setlog("fret=%d", fret);*/
    } else {
      setlog(L"get unhook ptr false.");
    }
  } else {
...
bool opDxGL::requestCapture(int x1, int y1, int w, int h, Image &img) {
  ...
  if (GET_RENDER_TYPE(_render_type) == RENDER_TYPE::DX) { // NORMAL
    ...
    for (int i = 0; i < h; i++) {
      memcpy(img.ptr<uchar>(i), ppixels + (i + y1) * 4 * _width + x1 * 4,
             4 * w);
    }
  } else {
    ...
    for (int i = 0; i < h; i++) {
      memcpy(img.ptr<uchar>(i),
             ppixels + (_height - 1 - i - y1) * _width * 4 + x1 * 4, 4 * w);
    }
  }
