...
opWGC::opWGC()
    : device_(nullptr), item_(nullptr), framePool_(nullptr), session_(nullptr),
      d3dDevice_(nullptr), d3dDeviceContext_(nullptr), m_frameInfo() {}

opWGC::~opWGC() { UnBindEx(); }
...
long opWGC::UnBindEx() {
  if (framePool_) {
    try {
      ...
      setlog("Direct3D11CaptureFramePool::Close (0x%08X)",
  ...
  if (session_) {
    try {
      ...
      setlog("GraphicsCaptureSession::Close (0x%08X)",
             winrt::to_hresult().value);
    }
  }
  ...
  if (device_) {
    try {
      ...
      setlog("IDirect3DDevice::Close (0x%08X): %s", err.code().value,
             winrt::to_string(err.message()).c_str());
    } catch (...) {
...
bool opWGC::Init(HWND _hwnd) {
  ...
  try {
    const HRESULT hr = interop_factory->CreateForWindow(
        _hwnd, winrt::guid_of<IGraphicsCaptureItem>(),
        reinterpret_cast<void **>(winrt::put_abi(item)));
    if (FAILED(hr))
      setlog("CreateForWindow (0x%08X)", hr);
  } catch (winrt::hresult_error &err) {
    setlog("CreateForWindow (0x%08X): %s", err.code().value,
           winrt::to_string(err.message()).c_str());
  } catch (...) {
  ...
  // 以下代码获取窗口大小，并从新初始化共享内存。
  // GetWindowRec方式获取的不准，缩放时
  Direct3D11CaptureFrame frame = {nullptr};
  // 窗口最小化时，可能获取不到， 最开始的时候也获取不到，
  // 尝试20次，如果尝试不成功就绑定失败
  int i = 20;
  while (!frame && i > 0) {
  ...
  try {
    ...
    setlog("bkdisplay::re bind share mem %s exception:%s", _shared_res_name,
           e.what());
  }
...
bool opWGC::requestCapture(int x1, int y1, int w, int h, Image &img) {
  ...
  for (size_t i = 0; i < desc.Height; i++) {
    memcpy(pDest + i * desc.Width * 4, pData + i * mappedResource.RowPitch,
           desc.Width * 4);
  }
  _pmutex->unlock();
  texture2D->Release();
  ...
  if (x1 + w > desc.Width || y1 + +h > desc.Height) {
    setlog("error w and h "
           "src_x=%d,w=%d,desc.Width=%d,src_y=%d,h=%d,desc.Height=%d",
           x1, w, desc.Width, y1, h, desc.Height);
    return false;
  }
  ...
  // 将数据拷贝到目标
  for (int i = 0; i < h; i++) {
    memcpy(img.ptr<uchar>(i), pDest + (y1 + i) * 4 * desc.Width + x1 * 4,
           4 * w);
  }
  _pmutex->unlock();
  return true;
