...
opBackground::opBackground()
    : _hwnd(0), _is_bind(0), _pbkdisplay(nullptr), _bkmouse(new opMouseWin),
...
long opBackground::BindWindow(long hwnd, const wstring &sdisplay,
                              const wstring &smouse, const wstring &skeypad,
                              long mode) {
  // step 1.避免重复绑定
  UnBindWindow();
  ...
  if (_pbkdisplay->Bind(hWnd, display) != 1 ||
      _bkmouse->Bind(hWnd, mouse) != 1 || _keypad->Bind(hWnd, keypad) != 1) {
    UnBindWindow();
    return 0;
  }
...
long opBackground::GetBindWindow() { return (long)_hwnd; }

long opBackground::IsBind() { return _pbkdisplay ? 1 : 0; }
...
long opBackground::GetDisplay() { return _display; }
...
long opBackground::RectConvert(long &x1, long &y1, long &x2, long &y2) {
  ...
  /*if (_pbkdisplay && (_display == RENDER_TYPE::NORMAL || _display ==
  RENDER_TYPE::GDI)) { x1 += _pbkdisplay->_client_x; y1 +=
  _pbkdisplay->_client_y; x2 += _pbkdisplay->_client_x; y2 +=
  _pbkdisplay->_client_y;
  }*/
...
bool opBackground::check_bind() {
  ...
  // 绑定前台桌面
  return BindWindow((long)::GetDesktopWindow(), L"normal", L"normal", L"normal",
                    0);
}
