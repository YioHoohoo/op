...
long winkeypad::KeyDown(long vk_code) {
  ...
  case INPUT_TYPE::IN_WINDOWS: {
    ...
    dwVKFkeyData |= 0 << 24;
    dwVKFkeyData |= 0 << 29;
    ret = ::SendMessageTimeout(_hwnd, WM_KEYDOWN, vk_code, dwVKFkeyData,
...
long winkeypad::KeyUp(long vk_code) {
  ...
  case INPUT_TYPE::IN_WINDOWS: {
    ...
    following table. 0 - 15 Specifies the repeat count for the current
    message.The value is the number of times the keystroke is autorepeated as a
    result of the user holding down the key. The repeat count is always one for
    a WM_KEYUP message. 16 - 23 Specifies the scan code.The value depends on the
    OEM. 24 Specifies whether the key is an extended key, such as the right -
    hand ALT and CTRL keys that appear on an enhanced 101 - or 102 - key
    keyboard.The value is 1 if it is an extended key; otherwise, it is 0. 25 -
    28 Reserved; do not use. 29 Specifies the context code.The value is always 0
    for a WM_KEYUP message. 30 Specifies the previous key state.The value is
    always 1 for a WM_KEYUP message. 31 Specifies the transition state.The value
    is always 1 for a WM_KEYUP message.*/
    ...
    dwVKFkeyData |= 0 << 29;
    dwVKFkeyData |= 3 << 30;
    ret = ::SendMessageTimeout(_hwnd, WM_KEYUP, vk_code, dwVKFkeyData,
                               SMTO_BLOCK, 2000, nullptr);
    if (ret == 0)
      setlog("error code=%d", GetLastError());
    break;
