...
std::wstring libop::Ver() { return _T(OP_VERSION); }
...
void libop::GetPath(std::wstring &path) { path = m_context->curr_path; }

void libop::GetBasePath(std::wstring &path) { path = opEnv::getBasePath(); }

void libop::GetID(long *ret) { *ret = m_context->id; }

void libop::GetLastError(long *ret) { *ret = ::GetLastError(); }
...
void libop::CapturePre(const wchar_t *file, LONG *ret) { *ret = m_context->image_proc.Capture(file); }
...
void libop::GetForegroundFocus(long *rethwnd) { *rethwnd = (LONG)::GetFocus(); }

void libop::GetForegroundWindow(long *rethwnd) { *rethwnd = (LONG)::GetForegroundWindow(); }

void libop::GetMousePointWindow(long *rethwnd) { m_context->winapi.GetMousePointWindow(*rethwnd); }

void libop::GetPointWindow(long x, long y, long *rethwnd) { m_context->winapi.GetMousePointWindow(*rethwnd, x, y); }
...
void libop::GetWindow(long hwnd, long flag, long *nret) { m_context->winapi.GetWindow(hwnd, flag, *nret); }
...
void libop::SendPaste(long hwnd, long *nret) { *nret = m_context->winapi.SendPaste(hwnd); }

void libop::SetClientSize(long hwnd, long width, long hight, long *nret) {
  *nret = m_context->winapi.SetWindowSize(hwnd, width, hight);
}

void libop::SetWindowState(long hwnd, long flag, long *nret) { *nret = m_context->winapi.SetWindowState(hwnd, flag); }
...
void libop::SetWindowText(long hwnd, const wchar_t *title, long *nret) { *nret = ::SetWindowTextW((HWND)hwnd, title); }
...
void libop::RunApp(const wchar_t *cmdline, long mode, long *ret) { *ret = m_context->winapi.RunApp(cmdline, mode); }
...
void libop::SetClipboard(const wchar_t *str, long *ret) { *ret = m_context->winapi.SetClipboard(str); }

void libop::GetClipboard(std::wstring &ret) { m_context->winapi.GetClipboard(ret); }

void libop::Delay(long mis, long *ret) { *ret = ::Delay(mis); }

void libop::Delays(long mis_min, long mis_max, long *ret) { *ret = ::Delays(mis_min, mis_max); }
...
void libop::UnBindWindow(long *ret) { *ret = m_context->bkproc.UnBindWindow(); }

void libop::GetBindWindow(long *ret) { *ret = m_context->bkproc.GetBindWindow(); }

void libop::IsBind(long *ret) { *ret = m_context->bkproc.IsBind(); }

void libop::GetCursorPos(long *x, long *y, long *ret) { *ret = m_context->bkproc._bkmouse->GetCursorPos(*x, *y); }

void libop::MoveR(long x, long y, long *ret) { *ret = m_context->bkproc._bkmouse->MoveR(x, y); }

void libop::MoveTo(long x, long y, long *ret) { *ret = m_context->bkproc._bkmouse->MoveTo(x, y); }

void libop::MoveToEx(long x, long y, long w, long h, long *ret) {
  *ret = m_context->bkproc._bkmouse->MoveToEx(x, y, w, h);
}

void libop::LeftClick(long *ret) { *ret = m_context->bkproc._bkmouse->LeftClick(); }

void libop::LeftDoubleClick(long *ret) { *ret = m_context->bkproc._bkmouse->LeftDoubleClick(); }

void libop::LeftDown(long *ret) { *ret = m_context->bkproc._bkmouse->LeftDown(); }

void libop::LeftUp(long *ret) { *ret = m_context->bkproc._bkmouse->LeftUp(); }

void libop::MiddleClick(long *ret) { *ret = m_context->bkproc._bkmouse->MiddleClick(); }

void libop::MiddleDown(long *ret) { *ret = m_context->bkproc._bkmouse->MiddleDown(); }

void libop::MiddleUp(long *ret) { *ret = m_context->bkproc._bkmouse->MiddleUp(); }

void libop::RightClick(long *ret) { *ret = m_context->bkproc._bkmouse->RightClick(); }

void libop::RightDown(long *ret) { *ret = m_context->bkproc._bkmouse->RightDown(); }

void libop::RightUp(long *ret) { *ret = m_context->bkproc._bkmouse->RightUp(); }

void libop::WheelDown(long *ret) { *ret = m_context->bkproc._bkmouse->WheelDown(); }

void libop::WheelUp(long *ret) { *ret = m_context->bkproc._bkmouse->WheelUp(); }
...
void libop::GetKeyState(long vk_code, long *ret) { *ret = m_context->bkproc._keypad->GetKeyState(vk_code); }

void libop::KeyDown(long vk_code, long *ret) { *ret = m_context->bkproc._keypad->KeyDown(vk_code); }
...
void libop::KeyUp(long vk_code, long *ret) { *ret = m_context->bkproc._keypad->KeyUp(vk_code); }
...
void libop::KeyPress(long vk_code, long *ret) { *ret = m_context->bkproc._keypad->KeyPress(vk_code); }
...
void libop::SetDisplayInput(const wchar_t *mode, long *ret) { *ret = m_context->bkproc.set_display_method(mode); }

void libop::LoadPic(const wchar_t *file_name, long *ret) { *ret = m_context->image_proc.LoadPic(file_name); }

void libop::FreePic(const wchar_t *file_name, long *ret) { *ret = m_context->image_proc.FreePic(file_name); }
...
// 使用哪个字库文件进行识别
void libop::UseDict(long idx, long *ret) { *ret = m_context->image_proc.UseDict(idx); }
