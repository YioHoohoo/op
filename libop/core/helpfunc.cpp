...
std::string _ws2string(const std::wstring &ws) {
  ...
  char *m_char;
  int len =
      WideCharToMultiByte(CP_ACP, 0, ws.data(), nlen, NULL, 0, NULL, NULL);
...
string utf8_to_ansi(string strUTF8) {
  UINT nLen =
      MultiByteToWideChar(CP_UTF8, NULL, strUTF8.c_str(), -1, NULL, NULL);
  WCHAR *wszBuffer = new WCHAR[nLen + 1];
  nLen =
      MultiByteToWideChar(CP_UTF8, NULL, strUTF8.c_str(), -1, wszBuffer, nLen);
  wszBuffer[nLen] = 0;
  nLen = WideCharToMultiByte(936, NULL, wszBuffer, -1, NULL, NULL, NULL, NULL);
  CHAR *szBuffer = new CHAR[nLen + 1];
  nLen =
      WideCharToMultiByte(936, NULL, wszBuffer, -1, szBuffer, nLen, NULL, NULL);
  szBuffer[nLen] = 0;
  strUTF8 = szBuffer;
  delete[] szBuffer;
  delete[] wszBuffer;
...
long Path2GlobalPath(const std::wstring &file, const std::wstring &curr_path,
                     std::wstring &out) {
...
long setlog(const char *format, ...) {
  ...
  GetLocalTime(&sys);
  char tm[128];
  sprintf(tm, "[%4d/%02d/%02d %02d:%02d:%02d.%03d]", sys.wYear, sys.wMonth,
...
void split(const std::wstring &s, std::vector<std::wstring> &v,
           const std::wstring &c) {
