...
int Pipe::open(const string &cmd) {
  ...
  if (!CreateProcessA(NULL, buf, nullptr, nullptr, true, NULL, nullptr, nullptr,
                      &_si, &_pi))
    return -3;
  _reading = 1;
...
void Pipe::on_read(const string &info) { std::cout << info << std::endl; }

int Pipe::on_write(const string &info) {
  if (_reading) {
    unsigned long wlen = 0;

    return WriteFile(_hwrite2, info.data(), info.length() * sizeof(char), &wlen,
                     nullptr);
  }
  return 0;
}
