...
public:
  ...
  // 设置是否弹出错误信息,默认是打开
  // 0:关闭，1:显示为信息框，2:保存到文件,3:输出到标准输出
  ...
  // Process
  // inject dll
  void InjectDll(const wchar_t *process_name, const wchar_t *dll_name,
                 long *ret);
  ...
  //---------------------algorithm-------------------------------
  // A星算法
  void AStarFindPath(long mapWidth, long mapHeight,
                     const wchar_t *disable_points, long beginX, long beginY,
                     long endX, long endY, std::wstring &ret);
  //
  void FindNearestPos(const wchar_t *all_pos, long type, long x, long y,
                      std::wstring &ret);
  //--------------------windows api------------------------------
  // 根据指定条件,枚举系统中符合条件的窗口
  void EnumWindow(long parent, const wchar_t *title, const wchar_t *class_name,
                  long filter, std::wstring &ret);
  // 根据指定进程以及其它条件,枚举系统中符合条件的窗口
  void EnumWindowByProcess(const wchar_t *process_name, const wchar_t *title,
  ...
  // 根据指定的进程名字，来查找可见窗口
  void FindWindowByProcess(const wchar_t *process_name,
                           const wchar_t *class_name, const wchar_t *title,
                           long *ret);
  // 根据指定的进程Id，来查找可见窗口
  void FindWindowByProcessId(long process_id, const wchar_t *class_name,
                             const wchar_t *title, long *ret);
  // 查找符合类名或者标题名的顶层可见窗口,如果指定了parent,则在parent的第一层子窗口中查找
  void FindWindowEx(long parent, const wchar_t *class_name,
                    const wchar_t *title, long *ret);
  // 获取窗口客户区域在屏幕上的位置
  void GetClientRect(long hwnd, long *x1, long *y1, long *x2, long *y2,
                     long *ret);
  // 获取窗口客户区域的宽度和高度
  ...
  // 获取窗口在屏幕上的位置
  void GetWindowRect(long hwnd, long *x1, long *y1, long *x2, long *y2,
                     long *ret);
  // 获取指定窗口的一些属性
  ...
  void BindWindow(long hwnd, const wchar_t *display, const wchar_t *mouse,
  ...
  void Capture(long x1, long y1, long x2, long y2, const wchar_t *file_name,
               long *ret);
  // 比较指定坐标点(x,y)的颜色
  void CmpColor(long x, long y, const wchar_t *color, double sim, long *ret);
  // 查找指定区域内的颜色
  void FindColor(long x1, long y1, long x2, long y2, const wchar_t *color,
                 double sim, long dir, long *x, long *y, long *ret);
  // 查找指定区域内的所有颜色
  void FindColorEx(long x1, long y1, long x2, long y2, const wchar_t *color,
                   double sim, long dir, std::wstring &retstr);
  // 根据指定的多点查找颜色坐标
  void FindMultiColor(long x1, long y1, long x2, long y2,
                      const wchar_t *first_color, const wchar_t *offset_color,
                      double sim, long dir, long *x, long *y, long *ret);
  // 根据指定的多点查找所有颜色坐标
  void FindMultiColorEx(long x1, long y1, long x2, long y2,
                        const wchar_t *first_color, const wchar_t *offset_color,
                        double sim, long dir, std::wstring &retstr);
  // 查找指定区域内的图片
  void FindPic(long x1, long y1, long x2, long y2, const wchar_t *files,
               const wchar_t *delta_color, double sim, long dir, long *x,
               long *y, long *ret);
  // 查找多个图片
  void FindPicEx(long x1, long y1, long x2, long y2, const wchar_t *files,
                 const wchar_t *delta_color, double sim, long dir,
                 std::wstring &retstr);
  //
  // 这个函数可以查找多个图片,
  // 并且返回所有找到的图像的坐标.此函数同FindPicEx.只是返回值不同.(file1,x,y|file2,x,y|...)
  void FindPicExS(long x1, long y1, long x2, long y2, const wchar_t *files,
                  const wchar_t *delta_color, double sim, long dir,
                  std::wstring &retstr);
  // 查找指定区域内的颜色块,颜色格式"RRGGBB-DRDGDB",注意,和按键的颜色格式相反
  void FindColorBlock(long x1, long y1, long x2, long y2, const wchar_t *color,
                      double sim, long count, long height, long width, long *x,
                      long *y, long *ret);
  // 查找指定区域内的所有颜色块, 颜色格式"RRGGBB-DRDGDB", 注意,
  // 和按键的颜色格式相反
  void FindColorBlockEx(long x1, long y1, long x2, long y2,
                        const wchar_t *color, double sim, long count,
  ...
  //
  void GetScreenData(long x1, long y1, long x2, long y2, size_t *data,
                     long *ret);
  //
  void GetScreenDataBmp(long x1, long y1, long x2, long y2, size_t *data,
                        long *size, long *ret);
  //
  ...
  //----------------------ocr-------------------------
  long SetOcrEngine(const wchar_t *path_of_engine, const wchar_t *dll_name,
                    const wchar_t *argv);
  // 设置字库文件
  ...
  void Ocr(long x1, long y1, long x2, long y2, const wchar_t *color, double sim,
           std::wstring &ret_str);
  // 回识别到的字符串，以及每个字符的坐标.
  void OcrEx(long x1, long y1, long x2, long y2, const wchar_t *color,
             double sim, std::wstring &ret_str);
  // 在屏幕范围(x1,y1,x2,y2)内,查找string(可以是任意个字符串的组合),并返回符合color_format的坐标位置
  void FindStr(long x1, long y1, long x2, long y2, const wchar_t *strs,
               const wchar_t *color, double sim, long *retx, long *rety,
               long *ret);
  // 返回符合color_format的所有坐标位置
  void FindStrEx(long x1, long y1, long x2, long y2, const wchar_t *strs,
                 const wchar_t *color, double sim, std::wstring &retstr);
  // 识别屏幕范围(x1,y1,x2,y2)内的字符串,自动二值化，而无需指定颜色
  void OcrAuto(long x1, long y1, long x2, long y2, double sim,
               std::wstring &ret_str);
  // 从文件中识别图片
  void OcrFromFile(const wchar_t *file_name, const wchar_t *color_format,
                   double sim, std::wstring &retstr);
  // 从文件中识别图片,无需指定颜色
  void OcrAutoFromFile(const wchar_t *file_name, double sim,
                       std::wstring &retstr);
  // 查找频幕中的直线
  void FindLine(long x1, long y1, long x2, long y2, const wchar_t *color,
                double sim, std::wstring &retstr);

  // 向某进程写入数据
  void WriteData(long hwnd, const wchar_t *address, const wchar_t *data,
                 long size, long *ret);
  // 读取数据
  void ReadData(long hwnd, const wchar_t *address, long size,
                std::wstring &retstr);
};
